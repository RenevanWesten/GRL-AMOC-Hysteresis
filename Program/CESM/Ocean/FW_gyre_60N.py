...
transport_gyre_all		= ma.masked_all(len(time_all))
...
for file_i in range(len(files)):
    ...
    for year_i in range(len(year)):
        ...
        for depth_i in range(len(depth)):
            ...
            salt_prime_depth	    = ma.masked_all((2, len(lon_t[0])))
        ...
        transport_gyre_all[file_i*len(year)+year_i]	= (-1.0 / 35.0) * np.sum(v_vel_prime * salt_prime * layer_field_u_area) / 10**6.0	

#-----------------------------------------------------------------------------------------

fig, ax	= subplots()

graph_control	= ax.plot(time_all, transport_gyre_all, '-k', linewidth = 1.5)
...
print(transport_gyre_all[2143])
#-----------------------------------------------------------------------------------------

print('Data is written to file')
fh = netcdf.Dataset(directory+'Ocean/FW_gyre_section_60N.nc', 'w')
